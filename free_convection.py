...
def free_convection_vertical_plate(NuL=None,
                                   RaL=None,
                                   L=None,
                                   k=None,
                                   Pr=None):
    ...
    if NuL is None and RaL < 1e9 and RaL > 1e4:
        ...
        NuL = 0.68 + (0.670 * RaL**0.25) / (1
                                            + (0.492 / Pr)**(9/16)
                                            )**(
                                                4/9
                                                )  # laminar w/ Pr correction
    elif NuL is None:
        NuL = (0.825
               + (0.387 * RaL**(1/6))
               / (1 + (0.492 / Pr)**(9/16))**(8/27)
               )**2  # turbulent with Pr correction

    h_hat = NuL * k / L
    return h_hat, NuL


def free_convection_horizontal_plate(NuL=None,
                                     RaL=None,
                                     L=None,
                                     k=None,
                                     A_s=None,
...
def free_convection_cylinder(NuD=None,
                             RaD=None,
                             D=None,
                             k=None,
                             Pr=None,
...
def rayleigh(T_surf, T_inf, L, nu, alpha, g=9.81):
    ...
    """
    # Approximate thermal expansion coefficient for ideal gases
    beta = 1 / ((T_surf + T_inf) / 2)
