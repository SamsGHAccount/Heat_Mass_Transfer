...
def conduction_1d(q=None, k=None, A=None, T1=None, T2=None, L=None):
    """
    Solves the 1D steady-state conduction equation for the unknown variable.
    q = k*A*(T1 - T2)/L
    Provide all variables except one, and the function will solve for the missing one.
    """
    # Define symbols
    q_sym, k_sym, A_sym, T1_sym, T2_sym, L_sym = sp.symbols('q k A T1 T2 L')
...
def conduction_radial(q=None, k=None, L=None, T1=None, T2=None, r1=None, r2=None):
    ...
    # Equation
    eq = sp.Eq(q_sym, 2 * sp.pi * k_sym * L_sym * (T1_sym - T2_sym) / sp.ln(r2_sym / r1_sym))
